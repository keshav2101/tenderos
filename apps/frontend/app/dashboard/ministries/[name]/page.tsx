...
export default function MinistryDetailPage() {
  ...
  const rawMin = params.name as string;
  const ministryName = decodeURIComponent(rawMin);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        console.error("Failed to load ministry tenders", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <span className="text-secondary text-xs font-semibold tracking-wider uppercase">Union Ministry Intelligence</span>
          <h1 className="text-3xl font-extrabold text-primary">{ministryName}</h1>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {loading ? (
            ...
            <div className="card p-8 text-center text-secondary border border-dashed border-slate-800">
              No active tenders found for this ministry.
            </div>
          ) : (
            <div className="space-y-4">
              {tenders.map((tender) => (
                  ...
                  <div className="card p-5 hover:border-blue-500/40 transition-all cursor-pointer bg-slate-900/20 border border-slate-800 rounded-xl space-y-3">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs text-secondary">
                      <span className="flex items-center gap-1">
                        ...
                        <span>{tender.department || "All Departments"}</span>
                      </span>
                      <span className="flex items-center gap-1">
        ...
        <div className="space-y-6">
          <div className="card p-5 bg-indigo-950/20 border border-indigo-500/20 rounded-xl space-y-4">
            <h3 className="text-md font-bold text-primary flex items-center gap-2 text-indigo-400">
              ...
              <span>Make In India (MII)</span>
            </h3>
            <p className="text-xs text-secondary leading-relaxed">
              Tenders under this ministry comply with GFR 2017 Rule 144(xi) and Make in India policy. Class-I Local Suppliers (≥50% local content) receive primary purchase preference, followed by Class-II Local Suppliers.
            </p>
            <div className="border-t border-slate-800 pt-3 space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-secondary">Preference Wave</span>
                <span className="text-emerald-400 font-semibold">Active ✔</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-secondary">Global Tenders</span>
                <span className="text-rose-400 font-semibold">Restricted (&lt;200Cr)</span>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5 bg-slate-900/40 border border-slate-800 rounded-xl space-y-4">
            <h3 className="text-md font-bold text-primary flex items-center gap-2">
              ...
              <span>Major Sub-Divisions</span>
            </h3>
            <div className="space-y-2 text-xs text-secondary">
              <div className="p-2.5 bg-slate-800/40 rounded-lg flex justify-between">
                <span>Central Public Enterprises</span>
                <span className="font-semibold text-primary">CPSEs</span>
              </div>
              <div className="p-2.5 bg-slate-800/40 rounded-lg flex justify-between">
                <span>Autonomous Bodies</span>
                <span className="font-semibold text-primary">R&D / Academic</span>
              </div>
              <div className="p-2.5 bg-slate-800/40 rounded-lg flex justify-between">
                <span>National Directorates</span>
                <span className="font-semibold text-primary">Direct Operations</span>
              </div>
            </div>
          </div>
        </div>
