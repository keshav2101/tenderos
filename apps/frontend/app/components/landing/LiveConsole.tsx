...
const PORTAL_COLORS: Record<string, string> = {
  "GeM": "#18794E", "CPPP": "#1F5A96", "IREPS": "#A16207",
  "Defence": "#B42318", "HAL": "#B42318", "BEL": "#B42318",
  "ONGC": "#475569", "BHEL": "#475569", "NTPC": "#475569",
  "State PWD": "#12355B", "State Health": "#1F5A96", "TN eProcure": "#12355B", "MahaGov": "#12355B",
};
...
function getColor(source: string) {
  return PORTAL_COLORS[source] || "#475569";
}
...
export default function LiveConsole({ tenders, isLoading, error, lastUpdated, onRefresh }: Props) {
  ...
  useEffect(() => {
    const t = setInterval(() => setTicker(x => x + 1), 3_000);
  ...
  return (
    <div className="bg-white border border-[#D9E1E8] rounded overflow-hidden shadow-2xs min-w-0 flex flex-col justify-between" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
      <div>
        {/* Header Bar */}
        <div className="px-4 sm:px-5 py-3 bg-[#F7F9FC] border-b border-[#D9E1E8] flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#18794E] animate-pulse" />
            <span className="font-mono font-bold text-[11px] uppercase text-[#18794E] tracking-wider">
              ● LIVE
            </span>
            <span className="text-[#64748B]">&middot;</span>
            <span key={ticker} className="text-[#475569] font-mono text-[11px]">
              Procurement network updating continuously (Updated {timeAgo(lastUpdated)})
            </span>
          </div>

          <button
            onClick={handleRefresh}
            className="text-[11px] font-bold text-[#1F5A96] hover:text-[#12355B] flex items-center gap-1 transition-colors"
          >
        ...
        {/* Responsive Waterfall Data Table */}
        <div className="overflow-x-auto min-w-full">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="border-b border-[#D9E1E8] bg-[#F1F5F9] text-[9px] font-bold uppercase tracking-wider text-[#475569]">
                ...
                {displayTenders.map((t, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 font-bold text-[#0B1F33] whitespace-nowrap">
                      ...
                      <td className="py-2.5 px-3 font-semibold text-[#0B1F33] max-w-[220px] truncate text-[11px]">
                        {t.title}
                      </td>
                      <td className="py-2.5 px-3 text-[#475569] max-w-[150px] truncate text-[11px]">
                        {t.department || t.ministry || "Ministry of Finance"}
                      </td>
                      <td className="py-2.5 px-3 font-mono font-bold text-[#0B1F33] text-right whitespace-nowrap text-[11px]">
                        {fmtVal(t.estimated_cost_lakhs)}
                      </td>
                      <td className="py-2.5 px-3 text-[#475569] text-center font-mono text-[11px] whitespace-nowrap">
                        {fmtDeadline(t.submission_deadline)}
                      </td>
                      <td className="py-2.5 px-3 font-mono font-bold text-[#18794E] text-center text-[11px] whitespace-nowrap">
                        {matchScore}%
                      </td>
                      <td className="py-2.5 px-3 text-center whitespace-nowrap">
                        <span className="text-[9px] font-bold font-mono text-[#1F5A96] bg-[#EFF6FF] px-1.5 py-0.5 rounded border border-[#BFDBFE]">
                          New
                        </span>
                      </td>
                    </motion.tr>
      ...
      {/* Footer bar */}
      <div className="px-4 sm:px-5 py-2.5 border-t border-[#D9E1E8] bg-[#F7F9FC] flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-mono text-[#18794E] font-bold">● Live stream connected</span>
        </div>
      </div>
    </div>
  );
