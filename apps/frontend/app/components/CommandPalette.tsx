...
export function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 bg-slate-900/40 backdrop-blur-xs animate-fade-in">
      <div 
        className="w-full max-w-2xl bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[80vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Input Bar */}
        <div className="flex items-center px-4 border-b border-slate-200 bg-slate-50/50">
          <Search className="w-4 h-4 text-blue-600 mr-3 flex-shrink-0" />
          <input
            ref={inputRef}
            type="text"
            ...
            className="w-full py-3.5 text-sm bg-transparent text-slate-900 placeholder-slate-400 outline-none"
          />
          {query && (
        ...
        <div className="overflow-y-auto p-2 space-y-4 max-h-[400px]">
          ...
          {query.trim() && (
            <button
              ...
              className="w-full p-3 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-between text-left hover:bg-blue-100/60 transition-colors"
            >
              <div className="flex items-center gap-2.5">
                <Sparkles className="w-4 h-4 text-blue-600" />
                <span className="text-xs text-blue-900 font-medium">
                  Execute Search for <strong className="text-blue-950">"{query}"</strong>
                </span>
              </div>
              ...
              {filteredNav.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${isSelected ? "bg-blue-100 text-blue-700" : "bg-slate-100 text-slate-500"}`}>
                      {item.category}
                    </span>
                  </button>
                );
            ...
            <div className="space-y-1">
              ...
              {filteredPrompts.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Sliders className={`w-4 h-4 ${isSelected ? "text-blue-600" : "text-slate-400"}`} />
                      <span>{item.label}</span>
                    </div>
        ...
        <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between text-[11px] text-slate-500">
          ...
          <div className="flex items-center gap-1 text-slate-600 font-semibold">
            <Zap className="w-3.5 h-3.5 text-blue-600" /> TenderOS Command Matrix
          </div>
        </div>
      </div>
    </div>
