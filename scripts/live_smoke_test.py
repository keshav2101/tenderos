...
test_route("Protected Connector API (Auth Guarded)", f"{BASE_BACKEND}/api/v1/connectors/status", 401)
...
try:
    ...
    req = urllib.request.Request(f"{BASE_BACKEND}/api/v1/auth/token", data=auth_data, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    with urllib.request.urlopen(req) as resp:
        ...
        test_route("Authenticated Connector Status API", f"{BASE_BACKEND}/api/v1/connectors/status", 200, headers={'Authorization': f'Bearer {access_token}'})
except Exception as e:
