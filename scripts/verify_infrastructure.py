...
import asyncio, json, os, socket, time, urllib.request, base64
...
async def check_postgres():
    try:
        ...
        record("PostgreSQL", True, f"{row['cnt']} tenders, latest: {str(row['latest'])[:10]}", ms)
    except Exception as e: record("PostgreSQL", False, str(e))
...
def check_redis():
    ...
    except Exception as e: record("Redis", False, str(e))
...
def check_qdrant():
    ...
    except Exception as e: record("Qdrant", False, str(e))
...
def check_minio():
    ...
    except Exception as e: record("MinIO", False, str(e))
...
def check_opensearch():
    try:
        ...
        ms = (time.perf_counter() - t0) * 1000
        s = data.get("status", "?")
        record("OpenSearch", s in ("green","yellow"), f"Cluster {s}, {data.get('number_of_nodes',0)} node(s)", ms)
    except Exception as e: record("OpenSearch", False, str(e))
...
def check_neo4j():
    ...
    except Exception as e: record("Neo4j", False, str(e))
...
def check_rabbitmq():
    try:
        ...
        data = json.loads(resp.read())
        ms = (time.perf_counter() - t0) * 1000
        record("RabbitMQ", True, f"v{data.get('rabbitmq_version','?')}, "
               f"messages_ready={data.get('queue_totals',{}).get('messages_ready',0)}", ms)
    except Exception as e: record("RabbitMQ", False, str(e))
...
def check_gemini():
    ...
    except Exception as e: record("Gemini API", False, str(e))
...
async def main():
    ...
    total = len(comps)
    results["summary"] = {"total":total, "passed":passed, "failed":total-passed, "score":f"{passed}/{total}"}
    print(f"\n{'='*60}")
