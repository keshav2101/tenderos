...
async def run_single_sync(source_id: str):
    ...
    if not job_id:
        logger.warning(
            "Could not create sync job log (connector not found in db)",
            source=source_id,
        )
        return
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            logger.info(
                "Connector sync job completed successfully",
                source=source_id,
                job_id=str(job_id),
                duration=duration,
            )
            ...
            logger.error(
                "Connector sync job failed with non-200 response",
                source=source_id,
        ...
        logger.error(
            "Exception during connector sync job",
            source=source_id,
            error=str(e),
            job_id=str(job_id),
        )
...
def cron_matches(cron_expr: str, dt: datetime) -> bool:
    ...
    try:
        return (
            match_field(minute, dt.minute)
            and match_field(hour, dt.hour)
            and match_field(dom, dt.day)
            and match_field(month, dt.month)
            and (match_field(dow, cron_dow) or (dow == "7" and cron_dow == 0))
        )
    except Exception:
        return True
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                async with pool.acquire() as conn:
                    ...
                    logger.info(
                        "Scheduler checking cron matches",
                    ...
                    for c in connectors:
                        ...
                        if running_job:
                            logger.info(
                                "Sync job already running; skipping trigger",
                                source=source_id,
                            )
                            continue
                        ...
                        consecutive_failures = len(recent_jobs) == 3 and all(
                            j["status"] == "failed" for j in recent_jobs
                        )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if job_dict["status"] == "running":
                logs.append(
                    f"[{datetime.utcnow().isoformat()}] INFO: Crawling pages and downloading specifications in progress..."
                )
