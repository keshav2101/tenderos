...
async def parse_natural_language_query(query: str) -> dict[str, Any]:
    ...
    for state in states_list:
        if re.search(r"\b" + re.escape(state) + r"\b", clean_query, re.IGNORECASE):
            ...
            clean_query = re.sub(
                r"\b(in|at|of)?\s*" + re.escape(state) + r"\b",
                "",
                clean_query,
                flags=re.IGNORECASE,
            )
    ...
    crore_match = re.search(
        r"(?:above|greater than|more than|\>\s*)\s*(\d+(?:\.\d+)?)\s*(?:crore|cr)",
        clean_query,
        re.IGNORECASE,
    )
    if crore_match:
    ...
    lakh_match = re.search(
        r"(?:above|greater than|more than|\>\s*)\s*(\d+(?:\.\d+)?)\s*(?:lakh|lk)",
        clean_query,
        re.IGNORECASE,
    )
    if lakh_match:
    ...
    crore_max_match = re.search(
        r"(?:below|less than|under|\<\s*)\s*(\d+(?:\.\d+)?)\s*(?:crore|cr)",
        clean_query,
        re.IGNORECASE,
    )
    if crore_max_match:
