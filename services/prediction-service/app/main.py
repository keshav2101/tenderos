...
async def get_predictions(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            return {"horizon_days": horizon_days, "predictions": predictions}
...
async def get_seasonal_patterns(ministry: str | None = None):
    try:
        ...
        async with pool.acquire() as conn:
            ...
            return {"ministry": ministry or "all", "patterns": patterns}

    except Exception as e:
        logger.error("Failed to fetch seasonal patterns from database", error=str(e))
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve seasonal patterns from database.",
        )
